...
import re
import sqlite3
conn = sqlite3.connect('error.sqlite')
c = conn.cursor()
...
def gather():
    for line in open('../error.log'):
        ...
        c.execute(s)
        if len(parts) > 1:
            c.execute("""insert OR IGNORE into ref (ref) values ('"""+parts[1]+"""')""")
        file_id = ref_id = 0
        c.execute('SELECT id from file WHERE file="'+parts[2]+'"')
        for row in c:
            file_id = row[0]
        
        if len(parts) > 10:
            sql = "SELECT id from ref WHERE ref='"+parts[1]+"'"
            c.execute(sql)
            for row in c:
                ref_id = row[0]
